...
#define NSS2STR(nsstr)			((char*)[(nsstr) UTF8String])
	...
	public:
		...
		const std::string getAppPath()			{ return mAppPath; }
		...
		bool open();
		bool saveas();
		void setDisplayFilename(const std::string & f);
		
		//
		// Presets
		void makePresets(int presetCount=CONFIG_PRESET_COUNT, bool _auto=true);
		void addParamToPreset(int id);
		int savePreset(char key);
		int loadPreset(char key);
		bool presetExists(char key);
	...
	protected:
		...
		std::string		mAppPath;				// The app folder
		std::string		mSavePath;				// Path being used for save/load
		std::string		mAppSupportFolder;
		std::string		fileExt;
		std::string		currentFileName;		// Full path of the file being used
