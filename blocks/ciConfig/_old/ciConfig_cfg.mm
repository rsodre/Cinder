...
namespace cinder {
	...
	ciConfig::ciConfig(ciConfig *_parent)
	{
		...
		mAppPath = getPathDirectory( app::getAppPath().string() );
		mSavePath = mAppPath;
		
		
		// Try to get Application Support folder
		...
		if ([possibleURLs count] >= 1) {
			...
			if (appSupportDir) {
				mAppSupportFolder = std::string([[appSupportDir path] UTF8String]) + "/" + mAppName + "/";
				printf(">>>> PATH App Support = [%s]\n",mAppSupportFolder.c_str());
			}
		}
	...
	void ciConfig::makePresets(int presetCount, bool _auto )
	{
		...
		for (int n = 0 ; ( n < presetCount && n < 10 ) ; n++)
		{
			...
			// delete old
			std::map<char,ciConfig*>::const_iterator it = presets.find(key);
			if ( it != presets.end() )
			{
				delete presets[key];
				presets.erase(key);
			...
			std::string fmt = currentFileName;
		...
		for ( it = presets.begin() ; it != presets.end(); it++ )
		{
			...
			switch (t)
			{
				...
				case CFG_TYPE_COLOR:
					...
					break;
				case CFG_TYPE_VECTOR2:
					pr->addVector2(id, this->getName(id).c_str(), this->getVector2(id), (char)this->getMin(id), (char)this->getMax(id));
					break;
				case CFG_TYPE_VECTOR3:
					pr->addVector3(id, this->getName(id).c_str(), this->getVector3(id), (char)this->getMin(id), (char)this->getMax(id));
					break;
				case CFG_TYPE_VECTOR4:
					pr->addVector4(id, this->getName(id).c_str(), this->getVector4(id), (char)this->getMin(id), (char)this->getMax(id));
					break;
	...
	// Save / load
	// i : preset number (1,2,...)
	int ciConfig::savePreset(char key)
	{
		std::map<char,ciConfig*>::const_iterator it = presets.find(key);
		if ( it != presets.end() )
		{
			...
			for (int id = 0 ; id < pr->params.size() ; id++ )
			{
				...
				switch (this->getType(id))
				{
					...
					case CFG_TYPE_VECTOR3:
					case CFG_TYPE_VECTOR4:
						for (int n = 0 ; n < this->getVectorCount(id) ; n++)
							pr->set( id, n, this->get(id, n) );
						break;
					case CFG_TYPE_STRING:
						pr->set( id, this->getString(id) );
						break;
					default:
	...
	int ciConfig::loadPreset(char key)
	{
		int r = 0;
		std::map<char,ciConfig*>::const_iterator it = presets.find(key);
		if ( it != presets.end() )
			...
			if (r > 0)
				for (int id = 0 ; id < pr->params.size() ; id++ )
				{
					...
					switch (this->getType(id))
					{
						...
						case CFG_TYPE_VECTOR4:
							for (int n = 0 ; n < this->getVectorCount(id) ; n++)
								this->set( id, n, pr->get(id, n) );
							break;
						case CFG_TYPE_STRING:
							this->set( id, pr->getString(id) );
							break;
						default:
	...
	bool ciConfig::presetExists(char key)
	{
		std::map<char,ciConfig*>::const_iterator it = presets.find(key);
		if ( it != presets.end() )
		{
			ciConfig* pr = (it->second);
			return fs::exists( pr->getFullFilename("") );
		}
		return false;
	}
	...
	//
	// Get file name
	std::string ciConfig::getFullFilename(const std::string & f, const std::string & path)
	{
		// Use current
		if ( f.length() == 0 )
		...
		if ( path.length() == 0 )
		{
			std::string appPath = this->getDefaultSavePath();
			os << appPath << f;
		}
		else
			os << path << f;
	...
	std::string ciConfig::getDefaultSavePath()
	{
		// Default = Besides the ".app"
		std::string appPath = mAppPath;
#ifdef STANDALONE
		// Inside app bundle
		appPath = getPathDirectory( App::get()->getResourcePath().string() ) + "Resources/";
#else
		// Besides the ".app"
		//appPath = mAppPath;
		// ~/Library/Application Support/<appname>
		appPath = mAppSupportFolder;
#endif
		return appPath;
	...
	bool ciConfig::open()
	{
		...
		std::string path = app::getOpenFilePath( mSavePath, extensions ).string();
		if ( path.length() )
		{
			mSavePath = path;
			this->useFile( path.c_str() );
		}
		return false;
	}
	bool ciConfig::saveas()
	{
		std::vector<std::string> extensions;
		extensions.push_back( fileExt );
		std::string path = app::getSaveFilePath( mSavePath, extensions ).string();
		if ( path.length() )
		{
			mSavePath = path;
	...
	{
		std::string fullFilename = this->getFullFilename(f);
		
		// Fom App Support folder
		if ( fullFilename.find(mAppSupportFolder) != std::string::npos )
		{
		...
		// Display file
		this->setDisplayFilename(fullFilename);
	...
	int ciConfig::saveFile(const std::string & f)
	{
		...
		// http://developer.apple.com/library/ios/#documentation/FileManagement/Conceptual/FileSystemProgrammingGUide/ManagingFIlesandDirectories/ManagingFIlesandDirectories.html
		if ( fullFilename.find(mAppSupportFolder) != std::string::npos )
		{
			if( ! fs::exists( mAppSupportFolder ) )
			{
				NSURL *url = [NSURL fileURLWithPath:[NSString stringWithUTF8String:mAppSupportFolder.c_str()]];
	...
	void ciConfig::setDisplayFilename(const std::string & f)
	{
		// Default configuration
		if ( f.find(mAppSupportFolder) != std::string::npos )
			mDisplayFileName = "<default>";
		// Make display filename
		else
		{
			std::ostringstream os;
			os << "[" << getPathFileName(f) << "]";
			mDisplayFileName = os.str();
		}
	}
